...
<Button href="/en/documentation/products/edge-application/cache-settings/" variant="secondary" text="Go to Cache Settings documentation"></Button>
...
Finally, if you've created a [device group](/en/documentation/products/guides/build/create-device-groups/), you can segment cache keys based on the device groups you created. To do so:
...
:::note
The `${uri}` variable may already be selected by default if you didn't activate **Application Acceleration**. For more information on Edge Application modules, see the [Edge Application documentation](/en/documentation/products/edge-application/).
:::
...
Before you run the Azion API, you'll need to [generate a personal token](/en/documentation/products/guides/personal-tokens/) for your account. You also need to [retrieve your edge application ID](/en/documentation/products/guides/build/configure-main-settings/) to run the following requests.

With your token and edge application ID in hand, follow the steps to create a new cache setting via API:

1. Run the following `PATCH` request in your terminal, replacing `[TOKEN VALUE]` with your personal token and the `<edge_application_id>` variable with the `id` of your edge application to activate the [Application Acceleration](/en/documentation/products/edge-application/application-acceleration/) module:
...
```
  ...
  | `cache_by_query_string` | Defines whether cache should be segmented into cache keys based on query strings. For all available options, see the [API documentation](https://api.azion.com). |
  ...
  | `adaptive_delivery_action` | Defines whether cache should be segmented into cache keys based on [device groups](/en/documentation/products/guides/build/create-device-groups/). |
  ...
  | `cache_by_cookies` | Defines whether cache should be segmented into cache keys based on cookies. For all available options, see the [API documentation](https://api.azion.com). |
