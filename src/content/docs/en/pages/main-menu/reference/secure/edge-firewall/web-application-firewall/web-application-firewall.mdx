...
The total amount of HTTP and HTTPS requests from a user or device made to edge applications running on Azion Edge Computing Platform that have the **Web Application Firewall (WAF)** module activated in **Edge Firewall**.

A request is billed whenever the request matches a rule enabled in **Rules Engine for Edge Firewall** with the **Set WAF** beahvior.
