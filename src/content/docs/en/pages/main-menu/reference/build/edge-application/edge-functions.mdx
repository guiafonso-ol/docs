...
Time, in milisseconds, of the execution of an edge function. It considers from the moment an edge function is called up to the moment it returns a response to **Edge Application**, that then returns a response to the user that made the request.
