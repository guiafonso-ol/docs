...
A object key é uma string de caracteres que serve como o identificador de objetos armazenados em buckets do **Edge Storage**. Por meio das ferramentas disponíveis, você pode entregar, modificar, recuperar e deletar um arquivo armazenado em um bucket usando a object key.

A object key não precisa corresponder exatamente ao nome original do arquivo no armazenamento local, nem mesmo incluir a extensão do arquivo original ou o caminho. No entanto, ao fazer o upload de um arquivo local para um bucket, é recomendado atribuir à object key o nome do arquivo para corresponder às convenções de armazenamento local. Por exemplo, para o arquivo local `folder/file.png`, a object key deve ser a mesma.
...
Um prefix, ou prefixo, é uma combinação de caminhos que simulam uma hierarquia de pastas. Como os buckets não podem ser organizados em pastas, você pode usar a barra (`/`) ao criar chaves para categorizar objetos do seu bucket em um prefix.
